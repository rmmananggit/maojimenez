	...
	<table class="table text-center table-hover table-striped">
		...
		<tbody>
			<?php
				...
				$i = 1;
				$qry = $con->query("SELECT `request_id`, user.user_id, `description`, `request_quantity`, `request_date`, `status_id`, fname, `mname`, `lname`, `suffix`, `product_name` FROM user INNER JOIN request INNER JOIN product where user.user_id = request.user_id AND product.product_id = request_id AND request.user_id = $user_id AND date(request_date) between '{$from}' and '{$to}' order by unix_timestamp(request_date) asc");
